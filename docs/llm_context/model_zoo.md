...
| Whisper real audio | examples/whisper_audio/* | HF whisper snapshot; ffmpeg; terminator.mp3 | MLX | Environment-blocked baseline | tests/integration/test_whisper_audio_real_mlx.py; test_whisper_audio_transpile_source.py |
...
| ResNet-18 | examples/model_zoo/* | torchvision | JAX/MLX smoke subsets | Tier 2 baseline pass | uv run python -m examples.model_zoo.transpile_smoke --target jax --model ResNet-18 |
...
- Real Whisper audio lane failed in this environment because host `ffmpeg` is broken.
