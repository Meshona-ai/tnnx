...
def test_cli_transpile_jax(tmp_path: str) -> None:
    onnx_path = Path(tmp_path) / "model.onnx"
    out_dir = Path(tmp_path) / "generated_jax"
...
def test_cli_transpile_jax_no_graph_ir(tmp_path: str) -> None:
    onnx_path = Path(tmp_path) / "model.onnx"
    out_dir = Path(tmp_path) / "generated_jax_no_graph_ir"
    _write_simple_onnx(onnx_path)
    result = run(
        [
            "uv",
