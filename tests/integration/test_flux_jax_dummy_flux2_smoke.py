...
def _run_flux_jax_dummy_flux2_demo():
    repo_root = Path(__file__).resolve().parents[2]
    if str(repo_root) not in sys.path:
        sys.path.insert(0, str(repo_root))
    from examples.flux import run_flux_jax_dummy_flux2_demo

    return run_flux_jax_dummy_flux2_demo


def test_flux_jax_dummy_flux2_smoke(tmp_path: Path) -> None:
    _ = pytest.importorskip("jax")
    run_flux_jax_dummy_flux2_demo = _run_flux_jax_dummy_flux2_demo()

    result = run_flux_jax_dummy_flux2_demo(tmp_path, steps=2)
    ...
    assert Path(str(result["transformer_module"])).exists()
    assert Path(str(result["vae_module"])).exists()
