...
def _load_module(path: Path):
    spec = importlib.util.spec_from_file_location("generated_jax_topology", path)
...
def test_jax_topology_scheduler_runtime_parity(tmp_path: Path) -> None:
    _ = pytest.importorskip("jax")

    module_path = emit_jax_module(_out_of_order_split_ir(), tmp_path / "generated_jax_topology")
