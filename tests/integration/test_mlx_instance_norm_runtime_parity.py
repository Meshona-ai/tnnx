...
def _load_module(path: Path) -> Any:
    spec = importlib.util.spec_from_file_location("generated_mlx_instance_norm", path)
...
def test_mlx_instance_norm_runtime_parity(tmp_path: Path) -> None:
    ...
    onnx_path = tmp_path / "mlx_instance_norm.onnx"
    onnx.save(model, onnx_path)

    ir, weights = load_onnx_to_ir(onnx_path)
    module_path = emit_mlx_module(ir, tmp_path / "generated_mlx_instance_norm")
    weights_path = tmp_path / "mlx_instance_norm_weights.npz"
    ...
    assert np.allclose(actual, expected, rtol=1e-5, atol=1e-5), (
        f"MLX instance norm parity failed: max_abs={np.max(np.abs(actual - expected))}"
    )
