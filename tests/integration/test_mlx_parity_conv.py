...
def _load_module(path: Path):
    spec = importlib.util.spec_from_file_location("generated_mlx_conv_model", path)
...
def test_mlx_parity_conv(tmp_path: str) -> None:
    ...
    assert np.allclose(out, ref, rtol=2e-5, atol=2e-6)


def test_mlx_parity_conv1d(tmp_path: str) -> None:
    mlx = pytest.importorskip("mlx.core")
    _ = mlx
