...
def test_qwen3_5_synthetic_example_jax(tmp_path: Path) -> None:
    torch = pytest.importorskip("torch")
    _ = pytest.importorskip("jax")
    _ = pytest.importorskip("transformers")
    from transformers import Qwen3_5ForCausalLM

    from examples.qwen.infer_qwen3_5_from_transformers_jax import (
    ...
    assert float(result["max_abs"]) <= 1e-3
    ...
    assert str(result["jax_version"]) != ""


def test_qwen3_5_real_example_jax(tmp_path: Path) -> None:
    if os.getenv("RUN_QWEN_JAX_E2E", "0") != "1":
        pytest.skip("Set RUN_QWEN_JAX_E2E=1 to run the full Qwen3.5 JAX path.")

    _ = pytest.importorskip("torch")
    _ = pytest.importorskip("jax")
    _ = pytest.importorskip("transformers")
    from examples.qwen.infer_qwen3_5_from_transformers_jax import run_demo
